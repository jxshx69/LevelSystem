...
diag_log format["[DATABASE LOAD] Loading player %1 (UID: %2)", _name, _playerUID];
...
diag_log format["[DATABASE LOAD] Most-Wanted loaded for %1", _name];
...
try 
{
	private _queryResult = format["getPlayerTitle:%1", _playerUID] call ExileServer_system_database_query_selectSingle;
	...
	if (!isNil "_queryResult") then 
	{
		if ((typeName _queryResult) isEqualTo "ARRAY") then 
		{
			if ((count _queryResult) > 0) then 
			{
				...
				{
					private _charArray = toArray _playerTitle;
					private _cleanArray = [];
					{
						if (_x != 34) then 
						{
...
{
	diag_log format["[DATABASE LOAD] Player Title Query Error: %1", _exception];
};

if (isNil "_playerTitle" || _playerTitle == "" || _playerTitle == "<null>") then 
{
	_playerTitle = "Neuling";
};

_player setVariable ["ExilePlayerTitle", _playerTitle, true];
diag_log format["[DATABASE LOAD] Player Title: '%1'", _playerTitle];
...
diag_log format["[LevelSystem Load] Loading level for %1 (UID: %2)", _name, _playerUID];

try 
{
	_levelData = format["loadPlayerLevel:%1", _playerUID] call ExileServer_system_database_query_selectSingle;
	
	diag_log format["[LevelSystem Load] DB Query result: %1 (Type: %2)", _levelData, typeName _levelData];
	
	if (!isNil "_levelData") then 
	{
		if ((typeName _levelData) isEqualTo "ARRAY") then 
		{
			if ((count _levelData) >= 2) then 
			{
				...
				diag_log format["[LevelSystem Load] ✅ Loaded from DB: Level %1, XP %2", _playerLevel, _playerXP];
			}
			else
			{
				diag_log "[LevelSystem Load] ⚠️ DB returned empty array, creating default entry";
				format["createPlayerLevel:%1", _playerUID] call ExileServer_system_database_query_fireAndForget;
			};
		}
		else
		{
			diag_log format["[LevelSystem Load] ⚠️ DB returned non-array: %1", typeName _levelData];
			format["createPlayerLevel:%1", _playerUID] call ExileServer_system_database_query_fireAndForget;
		};
	}
	else
	{
		diag_log "[LevelSystem Load] ⚠️ DB returned nil, creating default entry";
		format["createPlayerLevel:%1", _playerUID] call ExileServer_system_database_query_fireAndForget;
	};
}
catch 
{
	diag_log format["[LevelSystem Load] ❌ ERROR: %1", _exception];
	_playerLevel = 1;
	_playerXP = 0;
};
...
// ========================================
_player setVariable ["ExileLevel", _playerLevel, false];  // ← false = LOCAL!
_player setVariable ["ExileXP", _playerXP, false];

diag_log format["[LevelSystem Load] Variables set on player: Level %1, XP %2", _playerLevel, _playerXP];
...
// ========================================
// SEND TO CLIENT - LOAD PLAYER RESPONSE
// ========================================
[
	_sessionID, 
	"loadPlayerResponse", 
...
diag_log format["[LevelSystem Load] ✅ Sent to client %1: Level %2, XP %3", _name, _playerLevel, _playerXP];
...
diag_log format["[DATABASE LOAD] ✅ Player %1 loaded successfully with Level %2 (%3 XP)", _name, _playerLevel, _playerXP];
